...
def _get_visualization(sess_graph_path, value_feed_dict, input_tensor, layers, path_logdir, path_outdir, method = None):
    ...
    with tf.Graph().as_default() as g:
        with tf.Session(graph=g).as_default() as s:
            #
            if is_gradient_overwrite:
                # 当调用的是反卷积接口时，就对默认的session做一些操作，尽管后续的s没有被用到，但是这个还是有用的，
                # 具体什么有用呢，看完之后过来补充，OK现在看完了，作用是把之前save的model用默认的session来覆盖掉
                # 这里的with的作用是把Relu函数以及LRN函数用自定义的函数来替代，并且存到图里去
            ...
            for layer in layers:
                if layer != None and layer.lower() not in dict_layer.keys():
                    ...
                    is_success = _visualization_by_layer_type(
                        g, value_feed_dict, input_tensor, layer_type, method, path_logdir, path_outdir)
                else:
